...
def menu():
    ...
    choice = input(
        "Enter the number corresponding to the menu:\n"
    ...
    if choice == "1":
        ...
        if seeAllSubs() == 0:
            ...
            if back == "1":
                ...
                sys.exit()
        choice = input(
            "Enter the number corresponding to the substitute you want to "
            "delete.\n"
        )
...
def selectprod(choice):
    ...
    try:
        ...
        input("Press any key to back to main menu.\n")
        menu()
    print(Product.prodFromCat(SELECTED_CATEGORY))
    ...
    try:
        ...
        menu()
    try:
        SELECTED_PRODUCT.append(Product.prodFromCat(SELECTED_CATEGORY)
                                [choiceprod][0])
    except IndexError:
    ...
    if Product.prodFromCat(SELECTED_CATEGORY)[choiceprod][5] == "a":
        print(
            "The product you selected has the highest possible nutriscore. "
        ...
        print("The product you selected has no nutriscore. Please chose "
              "another one.")
        back = input(
            "1 : Back to products.\n"
            "2 : Back to categories.\n"
...
def selectsub(category, nutriscore, choice):
    ...
    if Product.allSubstitutes(category, nutriscore) == []:
        print(
            "The product you selected has no higher nutriscore substitute. "
            "Please chose another one"
        )
        back = input(
            "1 : Back to products.\n"
    ...
    try:
        SELECTED_PRODUCT.append(
            Product.allSubstitutes(category, nutriscore)[choicesub][0]
        )
    except IndexError:
        print("The key you pressed doesn't correspond to any product.\n")
        input("Press any key to back to main menu.\n")
...
def seeAllSubs():
    ...
    for elem in Substitute.getAllSubstitute():
        i = i + 1
        print(
            "Product substitued number " + str(elem[0]) + ":\n"
